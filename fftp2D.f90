...
!*****************************************************************
      SUBROUTINE fftp2d_create_plan(plan,nx,ny,fftdir,flags)
!-----------------------------------------------------------------
!
...
!     nx,ny  : the size of the dimensions of the input array [IN]
...
!-----------------------------------------------------------------
      ...
      CALL rfftwnd_f77_create_plan(plan%planr,1,nx,fftdir,flags)
      CALL fftw_f77_create_plan(plan%planc,ny,fftdir, &
                               flags+FFTW_IN_PLACE)
      plan%nx = nx
      plan%ny = ny
      ...
      CALL fftp2d_create_block(nx,ny,nprocs,myrank,plan%itype1, &
                              plan%itype2)
      RETURN
...
!*****************************************************************
      SUBROUTINE fftp2d_create_block(nx,ny,nprocs,myrank,itype1,itype2)
!-----------------------------------------------------------------
!
...
!-----------------------------------------------------------------
      ...
      DO irank = 0,nprocs-1
         CALL range(1,nx/2+1,nprocs,irank,ista,iend)
         CALL block2d(1,nx/2+1,jsta,ista,iend,jsta,jend, &
                     MPI_DOUBLE_COMPLEX,itemp1)
         itype1(irank) = itemp1
      END DO
      CALL range(1,nx/2+1,nprocs,myrank,ista,iend)
      DO jrank = 0,nprocs-1
         CALL range(1,ny,nprocs,jrank,jsta,jend)
...
!-----------------------------------------------------------------
      ...
      DOUBLE COMPLEX,   DIMENSION(plan%ny,ista:iend)      :: out 
      DOUBLE COMPLEX,   DIMENSION(plan%nx/2+1,jsta:jend)  :: c1
      DOUBLE COMPLEX,   DIMENSION(ista:iend,plan%ny)      :: c2
!      DOUBLE COMPLEX,   DIMENSION(plan%ny,ista:iend)      :: c3
      DOUBLE PRECISION, DIMENSION(plan%nx,jsta:jend)      :: in
...
!
      CALL fftw_f77(plan%planc,iend-ista+1,out,1,plan%ny,c2,1,plan%ny)
...
!-----------------------------------------------------------------
      ...
      DOUBLE COMPLEX,   DIMENSION(plan%ny,ista:iend)      :: in 
      DOUBLE COMPLEX,   DIMENSION(plan%nx/2+1,jsta:jend)  :: c1
      DOUBLE COMPLEX,   DIMENSION(ista:iend,plan%ny)      :: c2
      DOUBLE PRECISION, DIMENSION(plan%nx,jsta:jend)      :: out
      INTEGER, DIMENSION(0:nprocs-1)            :: ireq1,ireq2
      INTEGER, DIMENSION(MPI_STATUS_SIZE)       :: istatus
...
! 1D FFT in each node using the FFTW library
!
      CALL fftw_f77(plan%planc,iend-ista+1,in,1,plan%ny,c2,1,plan%ny)
!
! Cache friendly transposition
!
      DO ii = ista,iend,csize
